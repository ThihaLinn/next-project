...
export const imageUpload = multer({
    storage:multerS3({
        ...
        key:(res,file,cb) => {
            cb(
                null,
                `foodie-pos/msquarefdc-batch3/thiha-linn/${Date.now()}_${file.originalname}`
            )
        }
    })
}).single("file")
...
export const generateLinkForQRCode = (tableId: number) => {
  ...
  export const qrCodeImageUpload = async (tableId: number) => {
    try {
      ...
      const input = {
        Bucket: "msquarefdc",
        Key: `foodie-pos/msquarefdc-batch3/thiha-linn/qrcode/tableId-${tableId}.png`,
      ...
      return `https://msquarefdc.sgp1.cdn.digitaloceanspaces.com/foodie-pos/msquarefdc-batch3/thiha-linn/qrcode/tableId-${tableId}.png`;
    } catch (err) {
      console.log(err);
    }
  };